...
class DataContainer:
    ...
    def __init__(
        ...
        assert len(self.F) > 0
        if self.bias_id:
            assert len(self.F_pred) > 0

        if self.delta:
            assert len(self.F_dft) > 0
    ...
    def __getitem__(self, idx):
        ...
        data["F_pred"] = np.zeros([np.sum(data["N"]), 3], dtype=np.float32)
        data["F_dft"] = np.zeros([np.sum(data["N"]), 3], dtype=np.float32)
        ...
        for k, i in enumerate(idx):
            ...
            if self.bias_id:
                data["F_pred"][nstart:nend] = self.F_pred[s:e]

            if self.delta:
                data["F_dft"][nstart:nend] = self.F_dft[s:e]
    ...
    def convert_to_tensor(self, data):
        self.dtypes["F_pred"] = torch.float32
        self.dtypes["F_dft"] = torch.float32
        for key in data:
    ...
    def get_dtypes(self):
        ...
        dtypes_input["F_pred"] = torch.float32
        dtypes_input["F_dft"] = torch.float32
